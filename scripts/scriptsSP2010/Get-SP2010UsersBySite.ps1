...
$users = foreach ($site in $webapp.sites)
{
$site.RootWeb | select -ExpandProperty siteusers | select @{name="url"; expression={$site.url}},userlogin,displayname,issiteadmin,isdomaingroup,email
$site.dispose()
}
