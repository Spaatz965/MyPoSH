#requires -Modules AzureADPreview, Microsoft.Online.SharePoint.Powershell

function Set-XMSPOSiteAdmins {
    <#
.SYNOPSIS
...
.INPUTS
    TypeName: Microsoft.SharePoint.SPFolder (Example Object Type)
    The Microsoft .NET Framework types of objects that can be piped to the
    function or script. You can also include a description of the input 
    objects.
.OUTPUTS
    TypeName: GLW.SharePoint.WebApplication.Inventory (Example)
...
#>
    ...
    param (
        ...
        [Alias("SharePoint", "SSA")]
        [Switch]$SharePointServiceAdministrator,
        [Parameter(Mandatory = $false)]
        [Alias("CA", "GA", "GobalAdmin")]
        [Switch]$CompanyAdministrator
    )
    ...
    BEGIN {
        ...
        # TODO: Add test for "All" value as part of array
        # TODO: Test array of $Site to ensure valid current tenant URL's OR single value of "All"
        # TODO: Add support for O365 Groups

        If ( $Site -eq "All" ) {
            $Site = Get-SPOSites -Limit All | where-object { $_.Template -ne "GROUP#0" }
        }

        If ( $SharePointServiceAdministrator) {
            If ( -not (get-azureaddirectoryrole | Where-Object { $_.displayname -eq "SharePoint Service Administrator" }) ) { 
                $RoleTemplate = Get-AzureADDirectoryRoleTemplate | where-object { $_.displayname -eq "SharePoint Service Administrator" }
                Enable-AzureADDirectoryRole -RoleTemplateId $RoleTemplate.objectid
            }
            $ObjectID = (Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -eq "SharePoint Service Administrator" }).ObjectID
            $SharePointServiceAdministratorLoginID = "c:0t.c|tenant|$ObjectID"
        }

        If ( $CompanyAdministrator ) {
            $ObjectID = (Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -eq "Company Administrator" }).ObjectID
            $CompanyAdministratorLoginID = "c:0t.c|tenant|$ObjectID"
        }

        if ( $SharePointServiceAdministratorLoginID -and $CompanyAdministratorLoginID ) {
            $SetAdminBlock = {
                Set-SPOUser -Site $SiteCollection -LoginName $SharePointServiceAdministratorLoginID -IsSiteCollectionAdmin
                Set-SPOUser -Site $SiteCollection -LoginName $CompanyAdministratorLoginID -IsSiteCollectionAdmin
            }
        }
        elseif ( $SharePointServiceAdministratorLoginID -and (-not $CompanyAdministratorLoginID) ) {
            $SetAdminBlock = {
                Set-SPOUser -Site $SiteCollection -LoginName $SharePointServiceAdministratorLoginID -IsSiteCollectionAdmin
            }
        }
        elseif ( (-not $SharePointServiceAdministratorLoginID) -and $CompanyAdministratorLoginID  ) {
            $SetAdminBlock = {
                Set-SPOUser -Site $SiteCollection -LoginName $CompanyAdministratorLoginID -IsSiteCollectionAdmin
            }
        }
        elseif ( (-not $SharePointServiceAdministratorLoginID) -and (-not $CompanyAdministratorLoginID)  ) {
            ThrowError -ExceptionMessage "Must have at least one admin role switch enabled"
        }
    ...
    PROCESS {
        ...
        # TODO: Test for GROUP#0

        foreach ( $SiteCollection in $site ) {
            Invoke-Command $SetAdminBlock
        }
    } # PROCESS
